...
public class SpeedPowerUp extends Actor {
    ...
    public SpeedPowerUp(TextureAtlas atlas, float x, float y, float multiplier, float duration) {
        this.anim = new Animation<>(0.1f, atlas.findRegions("Powerups/Speed"), Animation.PlayMode.LOOP);
        ...
        for (Actor a : getStage().getActors()) {
            if (a instanceof Player) {
                ...
                if (p.getBounds().overlaps(bounds)) {
                    p.activateSpeedBoost(multiplier, duration);
                    remove();
                    break;
                }
            }
